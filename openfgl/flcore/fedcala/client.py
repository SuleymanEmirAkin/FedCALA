...
class FedCALAClient(BaseClient):
    def __init__(self, args, client_id, data, data_dir, message_pool, device):
        super(FedCALAClient, self).__init__(
    ...
    def get_features(self):
        """Computes the mean of model outputs to represent data distribution."""
        ...
        with torch.no_grad():
            for batch_data in self.task.train_dataloader:
                ...
                if isinstance(output, (list, tuple)):
                    output = output[0]
                features.append(output.mean(dim=0))
        return torch.stack(features).mean(dim=0).cpu().numpy()

    def execute(self):
        # Retrieve the model specifically intended for this client's cluster
        if f"weight_client_{self.client_id}" not in self.message_pool["server"]:
            return

        global_params = self.message_pool["server"][f"weight_client_{self.client_id}"]
        ...
        for _ in range(self.ala_epochs):
            for batch_data in self.task.train_dataloader:
                if self.rand_percent < 100 and np.random.rand() > (
                ...
                with torch.no_grad():
                    for i, (p_loc, p_glob, p_model) in enumerate(
                        zip(
                            local_params_detached,
                            global_params,
        ...
        with torch.no_grad():
            for i, (p_loc, p_glob, p_model) in enumerate(
                zip(local_params_detached, global_params, self.task.model.parameters())
            ):
                if self.weights[i] is not None:
    ...
    def send_message(self):
        self.message_pool[f"client_{self.client_id}"] = {
            ...
            "features": self.get_features(),  # Send feature representation for clustering
        }
